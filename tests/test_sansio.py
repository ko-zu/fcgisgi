...
class TestFastCGIConnection(unittest.TestCase):
    def test_request_started(self):
        ...
        # FCGI_BEGIN_REQUEST
        content = struct.pack(FCGI_BEGIN_REQUEST_BODY_FORMAT, 1, 1) # role=1 (RESPONDER), flags=1 (KEEP_CONN)
    ...
    def test_stdin_received(self):
        ...
        header = struct.pack(FCGI_HEADER_FORMAT, FCGI_VERSION_1, FCGI_STDIN, 1, len(stdin_content), 0)
        ...
        # EOF STDIN
        header_eof = struct.pack(FCGI_HEADER_FORMAT, FCGI_VERSION_1, FCGI_STDIN, 1, 0, 0)
